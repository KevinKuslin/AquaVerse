...
class _DivePageState extends State<DivePage> with TickerProviderStateMixin {
  ...
  void _buildSwimmersOnce(Size size) {
    ...
    //    - sort ids supaya stabil
    //    - bagi merata sepanjang width + jitter kecil
    for (final entry in _bands.entries) {
      ...
      for (int i = 0; i < n; i++) {
        ...
        // posisi merata + jitter kecil
        final jitter = (rnd.nextDouble() - 0.5) * step * 0.35;
        s.x = ((i + 1) * step + jitter).clamp(0.0, w);

        // arah random
        if (rnd.nextBool()) s.vx = s.vx.abs();
        else s.vx = -s.vx.abs();
      }
    }
  ...
  void _applySeparation(Size size) {
    // separation ringan per band: sort by x lalu cek tetangga (O(n log n) per band)
    final w = size.width;
    for (final entry in _bands.entries) {
      ...
      for (int i = 0; i < ids.length - 1; i++) {
        ...
        // min gap berdasarkan ukuran (scale)
        final base = 110.0; // cukup buat estimasi jarak
        ...
        // kalau berhadap-hadapan, flip supaya gak “menumpuk lama”
  ...
  void _onDragUpdate(DragUpdateDetails d) {
    final newDepth =
        (_depth - d.delta.dy * dragSensitivity / pxPerMeter).clamp(0.0, _maxDepth);
      ...
      pageBuilder: (_, __, ___) {
        return _BiotaRealOverlaySheet(
          ...
          onOpenDetail: () {
            ...
            // 2) buka detail setelah dialog ketutup (biar ga nge-stack aneh)
                      ...
                      children: [
                        ...
                        AnimatedBuilder(
                          ...
                          builder: (_, __) {
                            ...
                            const extra = 220.0;
                            if (rect.bottom < -extra || rect.top > size.height + extra) {
...
class _SwimLayer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // kumpulkan visible entries + sort by Y (biar layering natural)
    ...
    for (final entry in swimmersById.entries) {
      ...
      final anchorY =
          (s.depthMeters - depth) * _DivePageState.pxPerMeter + size.height * 0.18;
      final y = anchorY + s.yOffset - fishH / 2;

      if (y < -extra || y > size.height + extra) continue;

      visible.add(_VisibleFish(
        id: id,
        row: row,
        swimmer: s,
        top: y,
    ...
    // yang lebih bawah digambar belakangan -> terlihat di depan
...
class _ScanOverlay extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final chipLeft = (center.dx - 110)
        .clamp(12.0, MediaQuery.sizeOf(context).width - 232);
...
class _ScanPainter extends CustomPainter {
  ...
  _ScanPainter({required this.progress, required this.active, required this.done});
...
class _OceanBackground extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final top = Color.lerp(const Color(0xFF0EA5E9), const Color(0xFF0B1020), t)!;
    final bot = Color.lerp(const Color(0xFF083344), const Color(0xFF05070E), t)!;
...
/// === Overlay dialog (PREVIEW) ===
/// Perubahan:
/// - Hapus tombol "Lihat detail"
/// - Card bisa di-tap langsung untuk buka detail (dan auto close via onOpenDetail)
/// - UI lebih clean (judul + hint kecil + gambar)
        ...
        child: ClipRRect(
          ...
          child: BackdropFilter(
            ...
            child: Material(
              ...
              child: InkWell(
                onTap: onOpenDetail, // tap card -> langsung buka detail (auto close)
                child: Container(
